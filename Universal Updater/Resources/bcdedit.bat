...
echo [COMMAND]: bcdedit /store "%bcdPath%" /set {default} bootflow 0x802 >> %outputLog%
bcdedit /store "%bcdPath%" /set {default} bootflow 0x802

:: Query back to check
bcdedit /store "%bcdPath%" /enum {default}

:: Run bcdedit and parse the output for 'bootflow'
for /f "tokens=1,* delims=" %%A in ('bcdedit /store "%bcdPath%" /enum {default} ^| findstr /i "bootflow"') do (
    echo %%A
)
